...
router.get('/', async (req, res) => {
    try {
        ...
        for (let i = 1; i <= now.getMonth() + 1; i++) {
            ...
            meetingMonthLabels.push(`Month ${i}`);
        ...
        const docMonthCompareLabels = ['Prev Month', 'Next Month'];
        ...
        const monthlyLabels = Array.from({ length: current_Month + 1 }, (_, i) => `Month ${i + 1}`);
        ...
        res.render('dashboard/admin/index', {
            ...
            blogWeekCompareLabels: JSON.stringify(['Prev Week', 'Next Week']),
            blogWeekCompareCounts: JSON.stringify([lastWeekCount, thisWeekCount]),

            blogMonthCompareLabels: JSON.stringify(['Prev Month', 'Next Month']),
